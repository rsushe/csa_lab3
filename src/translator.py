...
variables = {}
string_current_address = 15001
variable_current_address = 15512
functions = {}
...
def translate(source_code: str) -> tuple[list[bytes], list[str]]:
    ...
    return convert_to_binary(opcodes)


def write_bin_code(target_code_file, code):
    with open(target_code_file, "wb") as fm:
        for byte in code:
            fm.write(byte)


def write_debug_code(target_debug_file, debug_code):
    with open(target_debug_file, "w") as fd:
        for line in debug_code:
            fd.write(line)


def main(source: str, target_code: str, target_debug: str):
    with open(source, encoding="utf-8") as f:
        source = f.read()

    bin_code, debug_code = translate(source)

    write_bin_code(target_code, bin_code)
    write_debug_code(target_debug, debug_code)
    print("source LoC:", len(source.split("\n")), "code instr:", len(debug))


if __name__ == "__main__":
    assert len(sys.argv) == 4, "Wrong arguments: translator.py <input_file> <target_file> <debug_file>"
    _, src, target, debug = sys.argv
    main(src, target, debug)
